    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
                ...
                <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'} `}>
                     {msg.image ?(
                        <img src={msg.image} alt=''className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />
                            ...
                            <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none' : 'rounded-lg-none'}`}>{msg.text}</p>
                     )}
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3 '>
            <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                ...
                className='flex-1 text-bottom p-3 border-none rounded-lg outline-none text-white placeholder-gray-400'></input>
                <input type='file' id='image' accept='image/png image/jpeg' hidden />
                <label htmlFor='image'>
                    <img src={assets.gallery_icon} alt='Gallery_icon' className='w-5 mr-2 cursor-pointer' />
                </label>
            </div>
            <img src={assets.send_button} alt='send_icon' className='w-7 cursor-pointer' />
        </div>
    </div>
  ):(
